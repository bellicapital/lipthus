...
class Notifier {
    ...
    // noinspection JSUnusedGlobalSymbols
    async toSubscribers(dbName, model, type, value, onlyUsers, params) {
        // si no se facilita asunto, no se envía email a los no usuarios
        if (!onlyUsers && !params.subject)
            onlyUsers = true;
        const subscribers = await this.site.app.subscriber
    ...
    async _process(item, subscribed, options = {}) {
        ...
        const subscribers = subscribed.map((s) => s.email);
        if (!options.testmode) {
