...
var lwRTE = function (textarea, options) {
	...
	this.controls	= {rte: {disable: {hint: 'Código fuente'}}, html: {enable: {hint: 'Editor visual'}}};

	$.extend(this.controls.rte, options.controls_rte || rte_toolbar);
	$.extend(this.controls.html, options.controls_html || html_toolbar);
	$.extend(this.css, options.css || {});

	if(document.designMode || document.contentEditable) {
		$(textarea).wrap($('<div></div>').addClass('rte-zone').width(this.width));		
		$('<div class="rte-resizer"><a href="javascript:;"></a></div>').insertAfter(textarea);
...
lwRTE.prototype.enable_design_mode = function() {
	...
	if($(self.textarea).attr('id'))
		self.iframe.id = 'rte_' + $(self.textarea).attr('id');
	...
	var doc = "<html><head>" + base + css + "</head><body " + style + " style='padding:5px; font-size: 12px;'>" + content + "</body></html>";
	...
	try {
		...
		// The design mode will be set once the editor is focused
	...
	$(self.iframe).parents('form').submit(
		function() {
			/* jj */
//			self.controls.rte.word.exec.apply(self);
			/* end jj */
...
lwRTE.prototype.disable_design_mode = function(submit) {
	...
	self.textarea && $(self.textarea).remove();	// Elimina posibles restos de un anterior submit con return false.
	...
	if(self.iframe.id)
		self.textarea.id = self.iframe.id.substr(4);
		
	if(self.iframe.title)
		self.textarea.name = self.iframe.title;

	$(self.textarea).val($(self.iframe).contents().find('body').html().replace(/(<br>)*$/, ''));// jj: añadido replace para quitar retornos de carro al final
...
lwRTE.prototype.create_toolbar = function(controls) {
	var self = this;
	var tb = $("<div></div>").addClass('rte-toolbar').width('100%').append($("<ul></ul>")).append($("<div></div>").addClass('rte-clear'));
	var obj, li;
...
lwRTE.prototype.create_panel = function(title, width) {
	...
	/* jj - Busca el posible ascendente con zIndex por si es un dialog */
	var zIndex = 'auto';
	
	$('.rte-zone').parents().each(function(){
	...
	var panel	= $('<div></div>').hide().addClass('rte-panel').css({left: left, top: top, zIndex: zIndex});
	$('<div></div>')
