...
function schemaGlobalMethods(schema) {
    ...
    schema.methods.getVar = async function (k, req, forceTranslate = true) {
        ...
        switch (schema.getTypename(k)) {
            ...
            case 'location':
                return new location_1.LipthusLocation(val);
            case 'Number':
            ...
            switch (schema.getTypename(k)) {
                ...
                case 'Multilang':
                    if (schema.tree[k].constructor.name === 'Array' && obj[k]) {
                        ...
                        obj[k].forEach((o, i) => obj[k][i] = new MlText(o, this.collection, k + '.' + i, obj._id, this.db.lipthusDb.site));
                    }
                    break;
            }
        });
