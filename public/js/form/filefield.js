...
$.fn.camera = function(options){
	...
	const methods = {
		init: function (options) {
			return this.each(function () {
				...
				if (o.value) {
					...
					if (s.type === 'image') {
						const path = '/bdf/' + s.schema + '/' + s.itemid + '/' + s.field + '/';
				...
				function onSelect(e, file) {
					...
					if (navigator.app) {
						...
						if (s.limit) {
							const $children = s.gallery.children('.thumb-container');
				...
				function onUploaded(e, file, r) {
					...
					const $thumb = $(findInGallery(file)).innerProgressBar(100);
		...
		appendThumbs: function (thumbs) {
			...
			return this.each(function () {
				...
				$.each(thumbs, function (i, thumb) {
					const $d = $('<div class="thumb-container"/>').appendTo(s.gallery).data('thumb', thumb);
					let field = s.field;
					...
					if (s.type === 'image') {
						...
						const $a = $('<a class="photoswipe" href="' + thumb.src() + '"></a>').appendTo($d);
						const img = new Image();
				...
				const $a = s.gallery.find('a.photoswipe');
				...
				if ($.fn.photoSwipe)
					...
					$a.click(function (e) {
						...
						$('<img src="' + this.href + '" style="position:fixed;top:0;left:0;display:none;z-index:99"/>').appendTo('body').load(function () {
							const $modal = $('<div style="width:100%;height:100%;position:absolute;top:0;left:0;background-color:#fff;opacity:.8"/>')
								.appendTo('body');
		...
		appendUploading: function (file) {
			...
			return this.each(function () {
				const s = $(this).data('fileField');

				const $d = $('<div class="thumb-container" style="width:200px;height:50px">Subiendo archivo...</div>').appendTo(s.gallery).data('thumb', file),
					field = s.field;
