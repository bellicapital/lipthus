...
var	rte_toolbar = {
	s1				: {separator: true},
	bold			: {command: 'bold', tags:['b', 'strong'], hint: 'Negrita'},
	italic			: {command: 'italic', tags:['i', 'em'], hint: 'Cursiva'},
	strikeThrough	: {command: 'strikethrough', tags: ['s', 'strike'] },
	underline		: {command: 'underline', tags: ['u'], hint: 'Subrayado'},
	s2				: {separator: true },
	justifyLeft   	: {command: 'justifyleft', hint: 'Justifica a la izquierda'},
	justifyCenter	: {command: 'justifycenter', hint: 'Centra'},
	justifyRight	: {command: 'justifyright', hint: 'Justifica a la derecha'},
	justifyFull		: {command: 'justifyfull', hint: 'Justifica'},
	s3				: {separator : true},
	indent			: {command: 'indent', hint: 'Añade sangría'},
	outdent			: {command: 'outdent', hint: 'Quita sangría'},
	s4				: {separator : true},
	subscript		: {command: 'subscript', tags: ['sub']},
	superscript		: {command: 'superscript', tags: ['sup']},
	s5				: {separator : true },
	orderedList		: {command: 'insertorderedlist', tags: ['ol'], hint: 'Lista ordenada'},
	unorderedList	: {command: 'insertunorderedlist', tags: ['ul'], hint: 'Lista desordenada'},
	s6				: {separator : true },
...
<select>\
	...
	image			: {exec: lwrte_image, tags: ['img'], hint: 'Imagen'},
	link			: {exec: lwrte_link, tags: ['a'], hint: 'Enlace' },
	unlink			: {command: 'unlink', hint: 'Elimina enlace'},
	s8				: {separator : true },
	removeFormat	: {exec: lwrte_unformat, hint: 'Elimina formato'},
	word			: {exec: lwrte_cleanup_word, hint: 'Limpia etiquetas de Word'},
	clear			: {exec: lwrte_clear, hint: 'Vacía el Documento'}
};

var html_toolbar = {
	s1				: {separator: true},
	word			: rte_toolbar.word,
	clear			: rte_toolbar.clear
};
...
<div class="rte-clear"></div>\
<p class="submit"><button id="rte-ok">Ok</button><button id="rte-cancel">' + $.lang('_CANCEL') + '</button></p>'
).show();
	...
	$('#rte-cancel', panel).click( function() { panel.remove(); return false; } );
	$('#rte-ok', panel).click( 
		function() {
...
function lwrte_image() {
	var self = this;
	var panel = self.create_panel('Insertar imagen', 385);
	panel.append('<p><label>URL</label><input type="text" id="rte-url" size="56" value=""><button id="rte-file">Subir</button></p>\
<div class="rte-clear"></div>\
<p class="submit"><button id="rte-ok">Ok</button><button id="rte-cancel">' + $.lang('_CANCEL') + '</button></p>').show();

	var url = $('#rte-url', panel);
	var upload = $('#rte-file', panel).upload( {
		autoSubmit: false,
		action: _ROOT + '/upload',
		onSelect: function() {
			var file = this.filename();
			var ext = (/[.]/.exec(file)) ? /[^.]+$/.exec(file.toLowerCase())[0] : '';
			if(!(ext && /^(jpg|png|jpeg|gif)$/.test(ext))){
				alert('Invalid file extension');
				return;
			}
			$('.submit', panel).addClass('loading').find('button').hide();
			this.submit();
		},
		onComplete: function(response) {
			...
			if(response.error && response.error.length > 0){
				$('.submit', panel).removeClass('loading').find('button').show();
...
function lwrte_link() {
	...
	panel.append('\
<p><label>URL</label><input type="text" id="rte-url" size="46" value=""><button id="rte-file">Adjuntar archivo</button></p>\
<div class="rte-clear"></div>\
<p><label>Title</label><input type="text" id="rte-title" size="30" value=""><label>Target</label><select id="rte-target"><option value="">default</option><option value="_blank">new</option></select></p>\
<div class="rte-clear"></div>\
<p class="submit"><button id="rte-ok">Ok</button><button id="rte-cancel">' + $.lang('_CANCEL') + '</button></p>'
).show();
	...
	$('#rte-ok', panel).click( 
		function() {
			var url = $('#rte-url', panel).val();
			var target = $('#rte-target', panel).val();
			var title = $('#rte-title', panel).val();

			if(self.get_selected_text().length <= 0) {
				alert('Selecciona el texto que quieres enlazar!');
				return false;
			}
