...
class ShoppingCart {
    constructor(req) {
         ...
         * @type boolean
         */
        this.deliver;//no asignar nada por defecto
        ...
        if (obj) {
            ...
            if (obj.items)
                obj.items.forEach((item, idx) => {
                    this.items[idx] = new ShopItem(item);
                });
    ...
    data4save() {
        ...
        this.items.forEach(item => data.items.push(item.data4save()));
    ...
    loadML() {
        ...
        if (this.mlLoaded)
            return Promise.resolve(req.ml.all);
