			...
			if (q.id) {
				return schema.findById(q.id).then(obj => {
					...
					if (q.direct)
						...
						finish(obj[q.m].apply(obj, q.a.concat([oldCompat])));
				});
			} else if (!schema[q.m])
				reject(new Error('Method "' + q.s + '.' + q.m + '" does not exists'));
			else
				finish(schema[q.m].apply(schema, q.a.concat([oldCompat])));
		});
	}
			...
			case 'object':
				if (a instanceof Array) {
					for(let i of Object.keys(a)) {
						a[i] = AjaxProcess.sanitizeArgs(a[i], req);
					}
				}
				break;
		}
...
module.exports = function(req, res){
	new AjaxProcess(req).process(res)
