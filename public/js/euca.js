...
$.addLang = function(a){$.extend(euca._lang, a);};
...
$.endLoadingMsg = function(){$('#fetchMsg').remove();};
	...
	if(!$.mobile){
		...
		$('<div style="padding:20px;text-align:center"><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>' + o.message +'</div>')
			.dialog({
				...
				modal: true,
				buttons: buttons,
				close: function(){$(this).remove(); o.onClick(false);}
			});
	} else {
		var $popup = $("#notiPopup");
...
$.prompt = function(options, complete){
	...
	if($.mobile){
		...
		var $field = $dg.find(o.fieldType + ':first');
...
$.jsLoaded(function(){
	var lastData
	,	csrf = $('head meta[name="csrf"]').attr('content')
	,	setup = {
		url: '/ajax',
		cache: true,
//		dataType: 'json',
	...
	if(euca.logLink !== false){//true si no se anula expresamente asignado false.
		//TODO: hacer que dispare sólo al abandonar el documento. Sino, genera errores en casos como photoSwipe,etc...
		$('a').click(function(){
...
/* Desactivado porque provoca demasiados errores
$.aj = function(c,m,a,s,t){
	if(s === 'post'){
		t = 'post';
		s = $.noop;
	}
	...
	var data = {classname: c[0], method: m, args: a};
