...
export function expressMongoStream(params: MongoFileParams, db: Db | any, req: any, res: Response) {
    const defaults = {
        expireDays: 60,
        mtime: new Date(),
		disposition: 'inline',
    ...
    const opt: MongoFileParams = Object.assign({}, defaults, params);
    let start = 0;
    ...
    const bucket = new GridFSBucket(db, {bucketName: opt.ns});
...
export interface MongoFileParams {
    id: any;
    mtime: Date;
    length: number;
    contentType: string;
